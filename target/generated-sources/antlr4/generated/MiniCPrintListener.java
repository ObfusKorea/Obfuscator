...
public class MiniCPrintListener extends MiniCBaseListener {
	...
	public void exitStmt(MiniCParser.StmtContext ctx) {
		...
		indentation = dots(if_count + while_count + fun_decl_count);
		/*stmt의 들여쓰기에 해당하는 문자열
		 * if, while과 같은 절 혹은 함수의 내부에 있는 문장에 대해 들여쓰기 처리를 하기 때문에
		 * 현재 ctx가 if, while, 함수 내부로 enter한 횟수를 세서 처리한다*/
		...
		if (ctx.getChild(0) instanceof MiniCParser.Compound_stmtContext) {
			/*compound_stmt일 경우
			 * indentation을 붙일 경우 중괄호의 시작 부분에
			 * 부적절한 들여쓰기 처리를 하게 되므로
	...
	public void exitCompound_stmt(MiniCParser.Compound_stmtContext ctx) {
		...
		newTexts.put(ctx, dots(if_count + while_count + fun_decl_count - 1) //	여는 중괄호 앞의 indentation
				+ s1 + "\n" + l_decl + stmt
				+ dots(if_count + while_count + fun_decl_count - 1) //	닫는 중괄호 앞의 indentation
				+ s2 + "\n");
		/*dots(if_count + while_count + fun_decl_count - 1)은
		 * 복합문을 여닫는 중괄호 앞에 출력되는 indentation에 해당한다
		 * 중괄호의 경우 자신이 속해있는 특수절이나 함수와 들여쓰기 정도가 같으므로
		 * 절, 함수 enter 합에 대해 1을 빼서 들여쓰기 단계를 맞춘다 */
	}

	@Override
	public void exitLocal_decl(MiniCParser.Local_declContext ctx) {
		String indentation = null, s1 = null, s2 = null, s3 = null, s4 = null, s5 = null, s6 = null;
		int count = ctx.getChildCount();
		
		indentation = dots(if_count + while_count + fun_decl_count); //	Local_decl의 indentation
		s1 = newTexts.get(ctx.type_spec()); //	type_spec
		s2 = ctx.getChild(1).getText(); //	IDENT
		switch (count) {
		case 3: // case : type_spec IDENT ';'
			s3 = ctx.getChild(2).getText(); // ';'
			newTexts.put(ctx, indentation + s1 + " " + s2 + s3 + "\n");
			break;
		case 5: // case : type_spec IDENT '=' LITERAL ';'
			s3 = ctx.getChild(2).getText(); //	'='
			s4 = ctx.getChild(3).getText(); //	LITERAL
			s5 = ctx.getChild(4).getText(); //	';'
			newTexts.put(ctx, indentation + s1 + " " + s2 + " " + s3 + " " + s4 + s5 + "\n");
			break;
		case 6: // case : type_spec IDENT '[' LITERAL ']' ';'
			s3 = ctx.getChild(2).getText(); //	'['
			s4 = ctx.getChild(3).getText(); //	LITERAL
	...
	public void exitIf_stmt(MiniCParser.If_stmtContext ctx) {
		...
		if (ctx.getChildCount() == 7) { //	case : IF '(' expr ')' stmt ELSE stmt
			...
			newTexts.put(ctx, s1 + " " + s2 + s3 + s4 + "\n" + s5
					+ dots(if_count + fun_decl_count + while_count - 1) //	else 키워드의 indentation
					+ s6 + "\n" + s7);
		}
		if_count--;
	}
